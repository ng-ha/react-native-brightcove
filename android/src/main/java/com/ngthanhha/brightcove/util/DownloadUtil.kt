...
object DownloadUtil {
  ...
  fun selectMediaFormatTracksAvailable(mediaDownloadable: MediaDownloadable, bundle: Bundle) {
    ...
    if (audio != null && audio.size > 1) {
      Log.v(
        TAG,
        "Alternate audio track download allowed for this video. Adding an \"alternate\" audio track"
      )
      ...
      Log.v(
        TAG,
        "Alternate audio track download allowed, but there were no \"alternate\" audio tracks to select."
      )
    }
    ...
    if (captions != null && captions.isNotEmpty()) {
      ...
      if (captions.size > 1) {
        Log.v(
          TAG,
          "Adding the first of the remaining caption tracks as the \"alternate\" caption track."
        )
        newCaptions.add(captions[1])
      } else {
        Log.v(
          TAG,
          "Captions size is not GT 1, no alternate captions will be downloaded, even though allowed."
        )
      }
    ...
    // Because if we always set the bundle to the MediaDownloadable, we're telling it to download every audio track and
    // every caption track.
    if (didListChange) {
